...
export const filterRequest = (e) => {
  ...
  if (el.checked) Object.keys(r).includes(el.name) ? r[el.name].push(el.value) : r[el.name] = [el.value];
  ...
  return async (dispatch, getState) => {
    filterProd = getState().homePage.products.filter((product) => {
      //console.log(Object.keys(r)); // ["brand", "chip_maker"]
      //console.log(r[Object.keys(r)]); // ["AMD", "NVIDIA"]

      return Object.keys(r).find(key => product.specifications.find((spec) => {
        return Object.keys(spec).find(s => s === key && r[Object.keys(r)] && r[Object.keys(r)].indexOf(spec[s].value) > -1);
      }))
    });
    ...
    if (getState().homePage.selectedFilters.length && el.name === getState().homePage.selectedFilters[0]) {
    ...
    } else if (getState().homePage.selectedFilters.length && getState().homePage.selectedFilters.indexOf(el.name) > 0) {
      ...
      getState().homePage.filterFields.map((filter) => {
        ...
        getState().homePage.filteredProducts.length &&
          getState().homePage.filteredProducts.filter((product) => {
            return product.specifications.filter((item) => { // {brand: {…}}
              return (
                Object.keys(item)[0] === filter.field &&
                result.push(Object.values(item)[0].value) &&
                hide.push(Object.values(item)[0].hide)
      ...
      getState().homePage.filterFieldsValues.map((obj, index) => {
        ...
        const temp = // ["Asus", "MSI", "PowerColor", "Sapphire"], [null, "NVIDIA"]...
          ...
          prop.map((p) => { // Asus
            ...
            if (getState().homePage.selectedFilters.indexOf(key) === 0 || (pos > -1 && pos && updateVal[pos] > 0)) {
              return p;
            }
